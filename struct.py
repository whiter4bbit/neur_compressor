...
class ComprLayer(NeurLayer):
    ...
    def teach(self, layers, X, alpha):
        ...
        dX = X_- X
        if alpha==-1:
            alpha = 1/float(1+sum(x*x for x in X_.m.tolist()[0]))
        W = self.W
...
class DecomprLayer(NeurLayer):
    ...
    def teach(self, layers, X, alpha):
        ...
        dX = X_- X
        if alpha==-1:
            alpha = 1/float(1+sum(y*y for y in Y.m.tolist()[0]))
        W_ = self.W
...
class CompressNetw(NeurNetw):
    ...
    def teach_layer(self, e_max, alpha, alpha_, max_iters):
        iters = 0
        error = 1e308
        c_alpha, c_alpha_ = None, None
        while error>e_max and iters<max_iters:
            E = []
            X_ = []
            t_start = time.time()
            for i in xrange(len(self.X)):
                ...
                c_alpha_ = t1[3]
                c_alpha = t2[3]
                E.append(sum(e*e for e in dX.m.tolist()[0]))
                X_.append(t2[1].m.tolist()[0])
            t = time.localtime(time.time()-t_start)
            ...
            for h in self.iter_handlers:
                teach_info = {'iter':iters, 'max_iters':max_iters, 'error':error,
                              'alpha_':c_alpha_, 'alpha':c_alpha, 'c_iter':iters}
                if error<=e_max:
                    teach_info['iter'] = max_iters
                h(teach_info)
        return (error,iters,detransf(X_))

    def teach(self, e_max, alpha, alpha_, max_iters):
        NeurNetw.re_init(self)
        print "teaching layers:"
        return self.teach_layer(e_max, alpha, alpha_, max_iters)
